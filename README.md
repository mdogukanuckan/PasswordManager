...
- [x] `Services/` klasörü özellik bazlı gruplandı (`Services/Auth/`, `Services/Vault/`) — namespace'ler klasörle eşleşecek şekilde güncellendi, `Http/`/`Exceptions/` cross-cutting klasörleri değişmeden kaldı
- [x] `HomePage`, gerçek bir vault listesine dönüştürüldü (`HomeViewModel` artık `CollectionView` ile `VaultItemListEntry` listesi gösteriyor, sayfa her göründüğünde `OnAppearing` ile `LoadVaultItemsCommand` otomatik tetikleniyor) — **uçtan uca runtime'da doğrulandı** (gerçek backend + PostgreSQL'e karşı)
- [x] Vault item ekleme ekranı (`AddVaultItemViewModel`/`AddVaultItemPage`) — Title/Username/Password/Notes formu, `IVaultItemMapper` ile şifreleyip `IVaultItemApiService.CreateAsync`'e gönderiyor, başarılı olunca relative `GoToAsync("..")` ile `HomePage`'e dönüyor (liste `OnAppearing` sayesinde otomatik yenileniyor) — **uçtan uca test edildi**
- [x] Vault item görüntüleme/detay ekranı (`VaultItemDetailViewModel`/`VaultItemDetailPage`) — listede bir item'a tıklayınca (`CollectionView.SelectedItem` + `IQueryAttributable` ile Shell üzerinden obje taşıma) zaten şifresi çözülmüş halde RAM'de duran veri, ekstra bir backend isteği olmadan detay sayfasına taşınıyor; şifre alanı varsayılan gizli, "Göster/Gizle" butonuyla açılabiliyor — **uçtan uca test edildi**
...
- **Düşük öncelikli gözlem — `HomePage` ilk yüklemede kısa bir gecikme:** Uygulama her yeniden başlatıldığında (backend hiç kapatılmadan bile) `HomePage`'in vault listesini ilk yüklemesi ~0,5-1sn sürüyor. Backend soğuk değilken de tekrarlandığı için muhtemelen client process'inin kendi JIT/ilk-TCP-bağlantısı soğuk başlangıç maliyeti — henüz kesin teşhis konulmadı (login sırasındaki Argon2id maliyetinden ayrı olduğu doğrulandı). Ayrıca `HomeViewModel.IsBusy` şu an `HomePage.xaml`'de hiçbir UI elemanına bağlı değil — bir `ActivityIndicator` eklenmesi planlanıyor, henüz yapılmadı.
